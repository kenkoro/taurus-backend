...
fun Route.createUser(
  ...
  post("/add-new/user") {
    ...
    val creatorSubject = call.parameters["creator_subject"] ?: run {
      call.respond(HttpStatusCode.BadRequest, "The creator must be provided")
      return@post
    }
    val creatorProfile = controller.user(creatorSubject)?.profile ?: run {
    ...
    if (creatorProfile != Admin) {
      ...
      val addedUser = controller.addNewUser(newUserWithHashedPassword, SaltWrapper(saltedHash.salt))
